...
function ctp_alert() : string {
	...
	if ( isset( $_SESSION['success'] ) ) {
		...
		$alert = '<div class="alert alert-success rounded-0">' . sanitize_email( $_SESSION['success'] ) . '</div>';

		unset( $_SESSION['success'] );

	}elseif ( isset( $_SESSION['error'] ) ) {
		
		$alert = '<div class="alert alert-danger rounded-0">' . sanitize_email( $_SESSION['error'] ) . '</div>';
